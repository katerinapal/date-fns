...
describe('compareDesc', function () {
  it('returns 0 if the given dates are equal', function () {
    var result = compareDesc(new Date(1989, 6 /* Jul */, 10), new Date(1989, 6 /* Jul */, 10));
    assert(result === 0);
  });

  it('returns 1 if the first date is before the second one', function () {
    var result = compareDesc(new Date(1987, 1 /* Feb */, 11), new Date(1989, 6 /* Jul */, 10));
    assert(result === 1);
  });

  it('returns -1 if the first date is after the second one', function () {
    var result = compareDesc(new Date(1989, 6 /* Jul */, 10), new Date(1987, 1 /* Feb */, 11));
    assert(result === -1);
  });
  ...
  it('accepts strings', function () {
    var result = compareDesc(new Date(1987, 1 /* Feb */, 11).toISOString(), new Date(1989, 6 /* Jul */, 10).toISOString());
    assert(result === 1);
  });

  it('accepts timestamps', function () {
    var result = compareDesc(new Date(1987, 1 /* Feb */, 11).getTime(), new Date(1989, 6 /* Jul */, 10).getTime());
    assert(result === 1);
  });
});